...
- **Наблюдаемость:** OAuth-часть невидима из UI (страницы работают и без единого
  ...
  читает их внешний суточный пинг; только булевы значения — число порталов из публичного маршрута
  не читается). Идентификатор портала в логи не попадает — только `portalHash`.
