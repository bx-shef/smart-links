...
- `app/` — Nuxt (авто-импорт):
  - `pages/` — `index.vue` (**публичный лендинг** на `/`, вне портала, `layout: false`),
    `privacy.vue` (**публичная** политика конфиденциальности на `/privacy` — вторая запись в
    `isPublicRoute`; новая публичная страница добавляется туда же в том же коммите) +
...
- **Правила пользовательских текстов** (закреплены тестами `tests/uiTexts.test.ts`):
  1. Простые слова. Внутреннего жаргона в интерфейсе нет: ни «сущность», ни «UF», ни «JSON»,
     ни «инфоблок», ни «плейсмент». Если понятие не обойти — объясняем словами пользователя
     («номер типа записи», «раздел, где лежит список»).
  2. Где ожидается клик — пишем «нажмите» и **называем сам элемент** так, как он подписан на
     экране. Ссылаться на кнопку, которой в этом состоянии нет, нельзя (иконочная кнопка своей
     подписи не имеет — называем её словами из `aria-label`).
  3. Ошибка отвечает на три вопроса: **где**, **что случилось**, **что делать дальше**. Диагноз
     ставим только тот, который код действительно проверил: «проверьте связь с интернетом» на
     ответ 403 — вранье.
  4. «Стоп» и «предупреждение» — разные вещи и выглядят по-разному. Кнопка называет действие
     («Настроить поле», а не «ОК»), прогресс говорит от нас («Отправляем…»).
  5. Ключ локали — всегда целый литерал. Собранный интерполяцией ключ невидим для проверок и
     ломает их в обе стороны (тест на это есть).
...
- Каждый `.md` в корне и `docs/` несёт `> Last reviewed: YYYY-MM-DD` под H1; оглавление
  `docs/README.md` полно в обе стороны. Гард — `tests/docsStructure.test.ts`.
- **Куда что писать** (правило эталона #490): **инвариант, запрет и цена нарушения одной строкой** —
  сюда, в `CLAUDE.md`; **разбор** («почему так, что пробовали, чем платим») — в `docs/PROCESS.md`;
  **состояние** («сделано / проверено вживую / осталось») — в `docs/PROJECT_MAP.md`. Разбор,
  положенный сюда, ничего не ломает сразу — поэтому его сюда и кладут, а файл, который грузится в
  каждую сессию, распухает. Сомневаешься — это разбор.
...
- **Раздел Списка (`IBLOCK_TYPE_ID`) хранится в настройке, но у админа не спрашивается.** Раньше в
  ...
  портал держит 0 списков в `lists` и 5 в `bitrix_processes`, а неверный тип — **жёсткая ошибка**,
  а не пустой ответ. То есть на таком портале половина функции («цель — элемент Списка») не
  работала вовсе и выглядела как поломка.
  ⚠ Спрашивать раздел у админа — тоже неверно: **названия разделов в REST и в меню портала не
  совпадают** (универсальные списки живут в «Автоматизация → Списки», а `bitrix_processes` — в
  «Лента → Ещё → Процессы»), поэтому любая наша подпись отправляла часть админов не туда. Поэтому
  настройки предлагают **выбрать список**, объединяя `lists.get` по `ENUMERABLE_IBLOCK_TYPES`, и
  раздел пишут сами; для списка группы/проекта (его `lists.get` без id группы не отдаёт) админ
  вводит номер, а раздел резолвится через **`lists.get.iblock.type.id`** (проверено живьём:
  `IBLOCK_ID=23 → bitrix_processes`, несуществующий id → `null`).
  Резолвер — `app/utils/listsTarget.ts` (чистый, с тестами), фолбэк — `lists`, нестроковое
  значение из опций портала не роняет плейсмент.
  ⚠ **Открытие/создание записи Списка ещё не разведено по разделам:** `appSettings.getTargetPath`/
  `getNewTargetPath` строят путь обычных списков портала для всех разделов. Поиск и привязка
  работают везде, ссылка «открыть» для прочих разделов **не проверена** — настройки об этом
  предупреждают, живой прогон за владельцем.
...
- **Наблюдаемость:** OAuth-часть невидима из UI (страницы работают и без единого
  ...
  булевы флаги готовности и исход keep-alive (`failing`/`lostGrant` — `lostGrant` защёлкивается;
  снимок пишется в Postgres и переживает переработку спящего процесса платформой, иначе суточный
  пинг всегда будил бы девственно-зелёный инстанс; только булевы значения — число порталов из
  публичного маршрута не читается). Идентификатор портала в логи не попадает — только `portalHash`.
...
- **Пер-IP лимит на `/api/app-rating`** работает независимо от флага: каждый вызов тратит исходящий
  ...
  учитывается только при `APP_EDGE_TRUST_XFF=1`. ⚠ Счётчик пер-процессный: scale-out умножает
  лимит на N инстансов — прежде чем масштабировать, нужен общий счётчик (шаблон — `sharedRateLimit`
  эталона).
- **SSRF-гард — явный список зон** (`B24_ZONES`), а не `bitrix24.<любой TLD>`: свободный TLD может
  ...
  собран по наблюдениям, **сверить с живым Маркетом** (дешёвый источник — `status.bitrix24.com`,
  перечисляет ДЦ по зонам); расширяется через `B24_EXTRA_ZONES`.
