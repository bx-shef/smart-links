...
Приложение исполняется **внутри iframe портала** (обёртка `@bitrix24/b24jssdk`), своего сервера
и БД не имеет: авторизация — на стороне портала, настройки — в опциях приложения
(`app.option.*`), реалтайм — через pull. В портал загружается **статическим архивом**.
