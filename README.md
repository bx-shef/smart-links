...
политика конфиденциальности на `/privacy`, `/api/health`, серверное состояние попапа оценки
(`/api/app-rating`) и **вебхук установки Маркета** (`/api/b24/events`), который хранит
OAuth-токены установленных порталов в Postgres — `refresh_token` шифруется AES-256-GCM, детали в
[`docs/DATA_POLICY.md`](docs/DATA_POLICY.md). Без `DATABASE_URL` приложение работает: рейтинг
инертен, установка Маркета отвечает 503. Миграция к паритету эталона завершена кодом (S1–S4),
живой прогон на портале впереди — см. [`docs/SERVER_MIGRATION.md`](docs/SERVER_MIGRATION.md).
