...
Цель связи — **любая запись CRM** (лид, сделка, смарт-процесс, счёт) **или элемент Списка**;
в роли источника сейчас поддерживается **Сделка**.

⚠ Приложение в разработке: цели-резолверы поддерживают лид, сделку, счёт и смарт-процессы
([#26](https://github.com/bx-shef/smart-links/issues/26) закрыт кодом 2026-08-21; открытие карточек
новых типов на живом портале ещё не проверялось). Что работает сегодня, а что входит в релизный
объём — в [`docs/PROCESS.md`](docs/PROCESS.md).
