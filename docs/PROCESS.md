...
**Поддерживаемые цели.** `entityMode` — `crm` или `lists`. Продукт связывает карточку с **любой
записью CRM — лидом, сделкой, смарт-процессом, счётом — или с элементом Списка** (решение владельца
2026-08-03; приложение в разработке, набор целей — часть релизного объёма).

**Состояние кода на 2026-08-21** ([#26](https://github.com/bx-shef/smart-links/issues/26) закрыт
кодом): в режиме `crm` поддержаны **лид, сделка, счёт и смарт-процессы**. Пути строит чистый
`app/utils/crmTargetPath.ts` (с тестами): лид и сделка — именованные маршруты, счёт (`entityTypeId`
31) и смарт-процессы (динамические типы, id ≥ 128 — реальные порталы выдают и 1256) — универсальный
`/crm/type/<entityTypeId>/details/`; схема снята с живых билдеров эталона. Непредлагаемые легаси-типы
(контакт, компания, предложение, старый счёт) честно резолвятся в `''` — эталон доказал живьём, что
универсальный маршрут предложение НЕ открывает, и «живая» ссылка в 404 хуже неработающей кнопки.
Редактор предлагает три статических типа + **смарт-процессы по названию** (`crm.type.list`,
fail-soft: без прав или на тарифе без смарт-процессов — живая проверка 2026-08-21 дала
`FEATURE_NOT_AVAILABLE_ON_CURRENT_PLAN` — остаются три статических типа и подпись почему).
Фильтры по компании/контакту работают для всех типов: поля целей задаются кодами `crm.item.*`
(camelCase), одинаковыми у лида, сделки, счёта и смарт-процессов с включёнными клиентами.
⚠ **Открытие карточек новых типов на живом портале не проверялось** — пункты добавлены в
`MANUAL_TEST_PLAN.md`.

Перечень целей уйдёт на витрину Маркета, а модерация не
публикует решение, «если функционал не соответствует заявленному в названии и описании». Кодом
обещание закрыто (#26); фактом оно станет после живого прогона (`MANUAL_TEST_PLAN.md`, блок 3).
...
3. Если приёмник пуст → режим **подбора**: `preLoadData()` грузит список кандидатов из
   ...
   `filterTitle`. Поиск в двух режимах устроен по-разному: в CRM — один вызов с OR-блоком
   «точный id ИЛИ подстрока названия», в Списках OR нет — два прохода (точный ID, затем
   `%NAME`) с объединением и дедупликацией. Формы фильтров — `app/utils/targetSearch.ts`,
   слияние проходов — `app/utils/listsSearch.ts` (оба под тестами).
...
при её получении перечитывает опции приложения и данные поля — **с разбросом до 4 секунд и
склейкой одновременных загрузок** (`app/utils/loadCoalesce.ts`, гард `tests/loadCoalesce.test.ts`):
команда будит каждую открытую карточку портала разом, и без разброса одно сохранение админа
устраивало бы залп в лимит запросов портала. Команду рассылает страница настроек после
сохранения.
