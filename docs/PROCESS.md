...
⚠ **Состояние кода на 2026-08-03.** В режиме `crm` резолверы путей
(`appSettings.getTargetPath`/`getNewTargetPath`) знают **только Сделку** (`EnumCrmEntityTypeId.deal`),
остальные CRM-типы проваливаются в `return ''`; редактор настроек соответственно предлагает один
CRM-тип. Пустой путь означает, что «открыть» и «создать» не сработают, то есть ветка недостижима
целиком, а не деградирует частично. Расширение — [#26](https://github.com/bx-shef/smart-links/issues/26):
у динамических типов (смарт-процесс, счёт) путь строится по `entityTypeId`, а не фиксированной
строкой, как у сделки, поэтому это отдельная ветка, а не пятая копия `case`.

Оговорка важна не для порядка в документах: перечень целей уйдёт на витрину Маркета, а модерация не
