...
- **SSR/пререндер по образцу эталона — сделано:** приведено к схеме `ai-price-import`:
  ...
  - **SDK не попадает в entry-чанк лендинга.** Плагин `@bitrix24/b24jssdk-nuxt` отключён (он
    ...
    чанк (296 КБ), который лендинг лишь `prefetch`-ит в простое. Правило на будущее: статических
    импортов `@bitrix24/b24jssdk` в `app.vue`, layout'ах, middleware и плагинах быть не должно
    (в страницах — можно, они грузятся своим чанком).
  ...
  - Смоук served-процесса: все маршруты 200, легаси-`.html` дают 301 на новые пути, неизвестный
    путь — 404, лендинг отдаёт `<h1>` в HTML, ошибок в логе нет.
