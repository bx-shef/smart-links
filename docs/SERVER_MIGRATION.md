...
> ⚠ Номера задач/PR в таблице фаз не сверены с трекером (аудит 2026-07-29 нашёл расхождения с git-историей — минимум ссылка S4 выглядит неверной); история мержей — источник правды.
...
- ⚠ **Однопроцессность.** Таймеры обслуживания — обычный `setInterval` без лока: наш деплой — один
  ...
  одновременно ротировать один портал. То же с пер-IP лимитом (`server/utils/rateLimit.ts`) и
  снимком keep-alive для `/api/health`: оба живут в памяти процесса, и N инстансов означают лимит
  ×N и N разных снимков — прежде чем масштабировать, нужен общий счётчик (эталон решает это
  Redis-окном, `sharedRateLimit.ts`/`windowCounterRedis.ts`).
